...
def run():
	...
	outputVehicle = "Sybil_1_A_2/vehicles_around_junction" + junction_ID + str(opt.run) +".txt"
...
if __name__ == "__main__":
	...
	outputFile = "Sybil_1_A_2/tripInfo" + str(opt.run) + ".xml"
	logFile = "Sybil_1_A_2/log" + str(opt.run) + ".xml"
	print 'Sybil IS IN THE HOUSE'
	
	seed = random.randint(1000, 10000)
    
	# Starting Traci
	#traci.start(['sumo-gui', "-c", "montreal_A_2.sumo.cfg", "-a", "montreal.additional.xml",  "--step-length", "0.1"])
	#to generate the tripInfo files in Sybil scenario for run number in the option
	traci.start(['sumo', "-c", "montreal_A_2.sumo.cfg", "--step-length", "0.1", "--seed", str(seed), "--tripinfo-output", outputFile, '--duration-log.statistics', 'True', '--collision.action', 'warn', '--log', logFile])
	run()
