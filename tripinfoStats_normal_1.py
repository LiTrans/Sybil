...
'''This script reads the content of the tripInfo files generated during the normal conditions simulations and builds the chart mean mean time loss vs simulation runs
...
def tripsStats(runs):		
	...
	for i in range(1, int(runs) + 1):
		file = 'Normal_1/tripInfo' + str(i) + '.csv'
		files.append(file)
	i=1
	for file in files:
		print "reading", file,"..."
		#Lire les vehicules affectes dans la zone
		inputVehicle = "Normal_1/vehicles_around_junction" + junction_ID + str(i)+ ".txt"
	...
	plt.grid(axis='both')
	plt.savefig("moyenne_Temps_perdu_Normal_1.svg")
	plt.show()
