	...
	while ct < 32400000:
		...
		if r is not None:
			...
			for x in temp:
				...
				if ((vehicle_list.count(x)==0) & (int(x)<1000000)):
					vehicle_list.append(x)
	outputVehicle = "Sybil_3_A_1/vehicles_around_junction" + junction_ID_1 + junction_ID_2 + junction_ID_3+ str(opt.run) +".txt"
...
if __name__ == "__main__":
	...
	# Starting Traci
	traci.start(['sumo-gui', "-c", "montreal3_A_1.sumo.cfg", "-a", "montreal.additional.xml",  "--step-length", "0.1"])
