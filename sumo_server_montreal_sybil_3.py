	...
	while ct < 32400000:
		...
		if r is not None:
			...
			for x in temp:
				...
				if ((vehicle_list.count(x)==0) & (int(x)<1000000)):
					vehicle_list.append(x)
	outputVehicle = "Sybil_3/vehicles_around_junction" + junction_ID_1 + junction_ID_2 + junction_ID_3+ str(opt.run) +".txt"
...
if __name__ == "__main__":
	...
	outputFile = "Sybil_3/tripInfo" + str(opt.run) + ".xml"
	logFile = "Sybil_3/log" + str(opt.run) + ".xml"
